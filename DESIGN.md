...
- **Text in toolbar pills and the breadcrumb is ink only** (adopted at the Phase 0 review): weight ≥ 500, no ink-2, no blue. The Phase 0 stand measured ink-2 on regular glass over a dark cover at ~3:1 and blue lower still, and a pill travels over anything the canvas shows. Consequences: the "Edited 2h ago" meta leaves the pill and lives on paper in the title block (Phase 2), where the head says when a page was edited and speaks only when a save fails — "Not saved" or "Conflict", never "Saving…" or "Saved", since a notes app keeping what is typed is not news; the breadcrumb parent is ink 500 followed by a chevron — blue stays for links on paper. Planned, not built: Phase 2 adds a **pill polarity** atom, `data-polarity="dark"` → dark material + light ink, so a pill over a dark cover follows the cover's luminance instead of fighting it.
...
- **What floats is sized by its content, not by the window.** The inset says how close a floating object may come to the edge; it never says the object reaches it. Every pill in the product is shrink-to-fit (breadcrumb, toolbar, toast) and so is the phone's bottom line, which is **two objects rather than one**: the **mobile tab bar** at the left inset, five slots of `--tabbar-slot` plus the row's 4px ends, and the **New circle** at the right inset, `--tabbar-h` square. Both stand `--tabbar-h` tall on one line. **One measurement gives the slot**: the widest label is still "Search" at 37.6 (Label 11/500), and "Tasks" measures 30.6 and does not take it. 9.2 of air on each side is 56.0, which is the 4px step. Five of them plus the ends is 288. **What the bar may take is the window less three insets and the circle**, those being its own inset, the circle's, and the air between them, which never closes tighter than the inset the two objects stand on. So the bar holds 288 down to 366, where the air is exactly 8, and the air is what the width spends: 32 at 390, 17 at 375, 72 at 430, 82 at 440, 409 at 767. Below 366 the slots give evenly instead, `(w − 78 − 8) / 5` a track, which is 46.8 at 320, and `minmax(44px, var(--tabbar-slot))` makes the tracks reach their floor at 306, where the bar is 228 and the circle still has its inset. Narrower again the two boxes stay 8 apart and what escapes is the bar's own content: `max-width` beats `fit-content`, so the box keeps shrinking past the 228 its five 44px tracks need, the row overflows a capsule that sets no `overflow`, and the last tab paints on bare canvas outside its own material and slides under the circle. Nothing ships there, so the floor stays where it is rather than being bought back by dropping the 44. New used to be the middle slot of six, a create standing between two destinations in a run of identical cells, and before that the bar divided the full width into five equal parts, 73px slots holding a 38px word, which reads as a backing laid under the screen rather than as an object standing on it. Measured per width by `e2e/mail-shots.spec.ts`.
- **Air around a word is not air between two words.** Two labels centred in equal tracks leave `track − (w₁ + w₂) / 2` between them, so the quantity to read is the widest adjacent **pair**, never the widest single label counted twice. Every slot carries a word now that the plus stands outside the bar, which gives four pairs and puts Tasks next to Pages. At the 56 slot Home↔Search measures 21.1, Search↔Tasks 21.9, Tasks↔Pages 24.3 and Pages↔Mail 28.4, so the minimum is still Home↔Search and the pair New left behind does not overtake it. A 20px minimum would be met by 54.9, so the check passes from 361 up and not below it: at 361 the track is 55 and Home↔Search measures 20.1, at 360 it is 54.8 and 19.9, and at 320 the track is 46.8 and the gap is 11.9, against 14.4 on the six-slot bar and 24.3 on the five-slot one that filled the window. 320 is the narrowest phone the e2e set still shoots, so unlike the 306 floor this one lands on a width that ships, and it is the measured cost of the circle taking 62 of the line. The 20 is a check the 56 slot passes above 361 and does not below it. It is not where the slot comes from, and the first draft of the bullet above said it was, on arithmetic for a pair the bar does not have.
- **The canvas runs between floating chrome, not only under it.** §7 gives the page canvas no edge band, so a paragraph passing the bottom line is drawn to the window's own edge and shows **between the two objects** as well as behind them: 32 of page at 390, 17 at 375, 8 from 366 down, and 72, 82 and 409 at 430, 440 and 767, the last width before the line is `display: none` and the sidebar takes over. Both objects stand on the 8px inset, inside the canvas's own 20px rule by 12, so the strip that used to show beside a centred capsule is gone and what replaces it is a column of the page between the bar and the circle. That is the better of the two: a strip 3px wide at 390 was a sliver of a word, and 32 is a run of text with an object at either end of it. The full-width bar had neither, by accident of its width rather than by anyone's decision, and least of all where it mattered most: a five-slot bar stretched across 767 is 751 wide with 150px slots, which is the failure this change is about, at its worst. Keeping the page hidden costs either a band on the canvas (§7's named ban) or a bar sized by the window (the bullet above), and text passing beside chrome is already what this product does at the top of a full-width page, where the article runs under the toolbar pills. So it stays visible: it is the two objects standing on the screen that make it so, and that is the change, not a side effect of it.
...
- **4px step**; canonical spacing 4/8/12/16/24/32/48. Control heights: 28 (dense row), 32 (field), 34 (primary circle), 36 (toolbar pill), 44 invisible touch minimum (`brain-touch-hit`); `brain-touch-min` is the 24 of WCAG 2.5.8, for a dense row whose 44 would overlap its neighbour.
...
On materialising: a blur ramp on the glass element itself (animating `filter` on top of `backdrop-filter`) is double compositing. Materialize through opacity + scale; the "glass has arrived" feeling comes from the edge-light, which appears at 60% progress on its own `::before` layer. **Radix surfaces (menus, popovers, dialogs, the palette) materialize through keyframes on `data-state`** — `materialize-in` / `materialize-out` / `materialize-edge` and `dialog-pop` in `globals.css`, `transform-origin` from the Radix variable — so Radix keeps the unmount timing and focus-return leases; framer is for what physically moves (sidebar, selection capsule, sheets, drag). A dialog leaves the way it was dismissed: `dialog-pop-out` retreats, scale back to .96 over 120ms, because nothing happened, and `dialog-commit` lets go, scale out to 1.02 over 200ms `--ease-out`, because what the surface was holding has just been put on the page underneath. The second is opt-in through `data-commit` on the Content, set before the close and left there until the next run, since the keyframe reads the attribute at the moment it starts. A second closed state on a shared class has to be named in the reduced-motion block beside the first (`globals.css`), or reduced motion keeps a scale in exactly one place. Everything the user drags (tree rows, stickers, sheets) runs on springs from the current value, never on CSS transitions. Theme change is a 200ms colour crossfade, no brightness jump.
...
2. **edge-fade** `<ScrollEdge variant="fade">` — inside glass panels (tree in the sidebar, lists in popovers, settings navigation, palette results): `mask-image` on the scroller itself, 12px at the top with `data-scrolled`, 20px at the bottom with `data-scroll-more`, eased over the same 160ms. No backdrop-filter — cheap. One exception, `data-edge="chips"`: a wrapped tile flow (Smart sort's chips) has no repeating row band and ends wherever the fold falls, so 20px under a 28px chip reads as one chip dimming rather than as a list continuing, and the bottom edge carries 34px, a whole chip and its gap.
...
- **Every `:hover` rule lives inside `@media (hover: hover)`** (`app/globals-hover.test.ts` holds it). A touch screen that can reach one paints it on the tap and keeps it until another tap takes it away, which reads as a selection that will not clear — the second half of "the greys are too bright on the phone". A finger gets `:active` instead, carrying the same tint: `.tint-hover` for anything with a fill, and its own rule on the four row families that have no press scale to fall back on (tree row, palette row, menu item, dialog row). What is deliberately outside the guard is everything in those selector lists that was never a pointer state — `:focus-visible`, `:focus-within`, Radix's `[data-highlighted]`, an open menu's `:has([data-state="open"])`, a selected row's `[data-selected]`.
...
| Kbd | `<Kbd>` (`ui/primitives.tsx`) | Kbd 11/500, ink-2, r6, 18 tall; fill from the surface — `--kbd-fill`: white .70 + rim inside a material, ink .05 on paper. A label is written once in Mac glyphs and the chip carries the Ctrl spelling beside it (`⌘⌥N` and `Ctrl+Alt+N`, `lib/shortcut-keys.ts`); `data-platform` on `<html>`, stamped before the first paint, shows one and hides the other, so the chip is right in the first frame on Windows and Linux and the hydrated tree still matches the server's. A `title` cannot be swapped in CSS: `useShortcutTitle` leaves the shortcut off until the platform is known. |
...
| Sidebar slot back row | `SlotBackRow` (`shell/sidebar.tsx`, `.brain-sidebar-back`) | while the sidebar slot hosts **Settings**, a quiet `tree-row` (28 capsule, Control 13/500, hover tint, press .97, 44px coarse target) above the slot content — chevron + **"Back"** — returns the way it came, through the surface's close semantics (history.back when entered in-app); the wordmark stays a secondary path. Mail had one of these too while the panel hosted its rail. It navigates itself from the head of its own column now (see §13), so the panel keeps the page tree with the Mail row marked in it, and there is no second surface in the slot to come back from. It said "Brain" until the word was standing twice in four lines, under the wordmark, naming the product the reader was already inside. A place-name cannot replace it either: the row follows history, so Settings opened out of Mail lands in Mail and "Pages" would be wrong exactly there. "Back" names no place and so cannot be wrong, and it matches the mobile settings header. The phone's Search and Pages used to draw a row like this one and now draw none: each owns a history entry, so the system Back and the iOS edge swipe leave them, and the Home tab owns the surface underneath. A surface that can be left three ways does not need a fourth drawn on it. |
| Mobile tab bar + New circle | `<MobileTabBar homeActive searchActive tasksActive pagesActive mailActive hidden searchRef pagesRef onHome onSearch onTasks onNew onNewTask onNewMessage onPages onMail>` (`mobile-tab-bar.tsx`, `.brain-mobile-tabbar` / `.brain-mobile-new`) | **two objects on one line** below md, both `--tabbar-h` (54) tall at the bottom inset. Left inset: the thick capsule at `--r-sheet` holding five slots of `--tabbar-slot` 56 on the row's 4px ends (Home, Search, Tasks, Pages, Mail), Linear at rest and Bold when current, the word under the glyph at Label 11, `brain-touch-min` and `.focus-inset` on every one. Right inset: the ink circle, 54 square, wearing the bare plus (§10 ban 13) and named "New" like the sidebar's, the surface's one ink-filled control (§2 → Primary). **It opens the New menu, as a sheet above the line**, the same menu the sidebar's circle opens, in the sheet form the When picker and the composer already take below md, so a phone that made a page outright while the desktop asked which of three things to make would be two products wearing one plus; hover lightens the fill by .08 and press scales .97 at 100ms, both on the circle inside the button so the hide's transform and the press's never contend for one property. §4 sizes the bar: the window less three insets and the circle, so it holds 288 down to 366 and gives its slots evenly below that, never under the 44 track floor. **They are siblings with no wrapper and one state**: `hidden` writes `data-hidden` on both, and both leave on the same 200ms transform and opacity, so a sheet, the keyboard or the palette takes the whole line rather than leaving a circle standing over an open sheet. The circle carries `fixed` beside `brain-touch-min`, which is the opt-out that helper's `:not(.fixed)` guard asks for; without it the touch rule pins the button to `position: relative` and it leaves the line. The circle comes last in the document, after Mail, so Tab reads the line left to right. `--tabbar-reserve` (`--tabbar-h` + two insets + the safe area) is what the canvas, the palette list, the Pages list and the mail column keep clear at their end. Search and Pages render their own copy inside their focus scope, so there is only ever one bar and one circle in the tree. Above md both are `display: none` and the sidebar takes over. New used to be the fourth of six slots inside the bar, a create between two destinations in a run of identical cells. |
...
| FloatingToolbar | `<FloatingToolbar container pages ai>` (`editor/floating-toolbar.tsx`, `role="toolbar"` `aria-label="Text formatting"`) | regular capsule 36 over a text selection: AI (capability-gated) · Bold · Italic · Strikethrough · Code · H1 to H3 · ¶ · Bullet list · Numbered list · Task · Quote · Table (that last group hidden inside a table) · Link to page · Colour. Task wraps the selected block(s) into an unchecked `- [ ] ` item and lifts back to a paragraph on a second press, `aria-pressed` carrying which state the line is in. |
| Menu / popover | classes `brain-menu` (on Radix `Content`), `brain-menu-item`, `brain-menu-icon`, `brain-menu-sep`, `brain-menu-label`, `brain-menu-sheet` — `tree/row-menu`, `page-actions-menu`, `new-menu`, `category-picker`, `emoji-picker`, `mail-nav` | regular material r14, padding 6, items 32 r8 ink 500, icon ink-2 → ink on highlight, tint on highlight, disabled .4; Label 11 section titles (sentence case); lists inside scroll under an edge-fade. Materialize on `data-state` (keyframes), exit 120ms. **The New menu is the one under the plus** (`new-menu.tsx`, both the sidebar head’s accent circle and the phone’s circle): a **New** group of `Task` (`checklist-linear`) and `Message` (`letter-linear`), the glyphs the Tasks tab and the Mail row already wear, then a `brain-menu-sep`, then a **Page** group holding Blank page and the five templates with their emoji, unchanged. It was a template menu labelled “New page”, which meant the one button in the product that says MAKE SOMETHING could make only one of the three things Brain keeps. **Message is absent, not dimmed**, when no connected account carries both `compose` and `send` (`mail-compose-available.ts`, one answer per session, shared): a row that can only apologise is worse than no row. Task and Message run through `useDeferredMenuAction`, because both put a caret somewhere and a caret moved inside Radix’s focus scope is one Radix takes back as the layer tears down. **Below md it is a sheet**: `brain-menu-sheet`, `100vw − 16` on the inset above the phone’s line, the composer’s grip, `SPRING_SHEET` in and the drag out, the material animating nothing and the hold keyframe the When picker’s sheet already uses. Same rows, same order, because a sheet is a shape and not a second menu. |
| When picker (D4) | `<TasksWhenPicker value today mode onPick trigger ariaLabel>` and `renderWhenPicker({ value, today, mode, reduce, onPick, onDone })` (`tasks-when-picker.tsx`) | the **thick** material at r14 and 264, which is seven 36px day cells on the menu material's own 6px padding. It takes the composer sheet's glass and blur rather than the menu's regular pair, on a pointer and on a phone alike, because a 7x6 grid of numbers is the one popover in the app where a stray glyph read through the material is indistinguishable from content (ban 2: a blurred layer is never the ground under a table). In deadline mode a **Deadline** caption stands at the top of the panel in the label register at ink-3, since the chip that opened it has already turned into `15 Sep` and the word appeared nowhere else but on the button that clears it. Rows in order: **Today** with the star, **This Evening** with the moon while the picked day is today (taken out of the tree otherwise, never dimmed, because a control that advertises itself and then refuses is the one shape a control must not have), a Monday-first **month grid** with today ringed in `--blue-rim` and the picked day an ink capsule, **Someday** under the grid with the archive box, a **Reminder** row opening two spinners at 09:00 (an hour and five minutes a press), each spinner its value with a vertical pair of 16px chevrons 6px to its right, up over down, and the colon 12px clear of the hour's pair; then **Clear** and **Done** in the foot, Done the quiet button carrying ink text at 600 and Clear beside it at ink-3, because ban 5 allows one ink fill per surface and the chosen day has it. Outside the month is ink-4 and before today is ink-3, both still pickable, since a deadline in the past is a real thing to record. Hover on a cell is `--blue-tint`, the list-row value of §8. The Reminder row waits for a day to be picked, because `lib/tasks/model.ts` refuses a clock without one, and Someday takes the clock and the evening off with it. A quick row (Today, This Evening, Someday, Clear) commits at once and closes: one tap, one write. The grid and the clock edit a local value and leave the panel standing, so a reminder can follow its day. Done, a press outside and the sheet's drag away commit that value once and close, Escape closes and discards it unless a quick row or Done has already answered (that write is on its way, so the key is inert), and a value that ends where it began writes nothing. A pick is discarded when the panel goes away without a close at all: a refetch, a row leaving, a route change. The row folds only after the panel has gone, and the write leaves with the picker already out of the document. **A press inside a panel the row opened is not a press on the row.** The panel is portalled to the end of the document and React carries its events up the tree the portal was DECLARED in, so a day cell reached the row's own handler, folded the row, and took the picker away before the day it was holding had gone anywhere: `openRow` asks containment of its own element before it answers anything, which covers every panel a chip opens rather than marking each portal. The chips themselves are inside the row, so each one carries `data-task-control`. The Category chip was the one that did not, and its press folded the row instead of opening the list. In a note the `+ Task` popover is this picker with one Inbox row above it, a `role="dialog"` named When on an `aria-haspopup="dialog"` trigger, and there Done and the quick rows create the task while a press outside, Escape or the note scrolling under it cancels, because a create has no Undo. It opens on the line's own record, so Clear on a line with a date clears it, and a route that refuses leaves the value one press away rather than spending it. Its Inbox row is handed down to the picker like every row the picker draws, so the no-op rule (a line already in the Inbox sends nothing), the waiting state and the refusal path are one rule rather than two. **One write leaves at a time and nothing queues behind it.** From the press to the answer the panel is `aria-busy`, its rows, its month label and its grid at ink-3 with the pointer off, and Done, Clear, the month arrows and the clock disabled; a press made there is neither sent nor remembered, since a word held back is filed a moment later out of sight of the reader who named it. Escape still closes and cancels nothing, the value being already with the route. An acceptance closes the panel; a refusal gives it back with the route's own reason on its one refusal line, and the same press writes again. The no-op rule measures against the last value a host TOOK, never against an optimistic one, so a refused value is one the panel can send again and a line this panel has moved to Today sends nothing on a second Today. A reason that arrives with the panel already gone (Escape, the note scrolling under it) is dropped rather than written into a node that has left the document: this popover has no toast by the decision two sentences up, and the line still wearing its ghost is what says nothing was filed. The row's chip is the other host and does have one: a refused reschedule there puts the row back and speaks in the shell's toast (`tasks-actions.ts`). A panel opened over a line that is already saving refuses its press out loud ("The last change is still saving") rather than taking it, because two records contending for one checkbox is the state with no honest reading. Keyboard: arrows a day, up and down a week, Home and End the ends of the week, PageUp and PageDown a month, Enter picks. Cells carry `.focus-inset`, the grid having no gap for a ring at +2 to land in. The grid travels 6px from the side the month came from over `DUR.base`, and stands still under reduced motion. The month label is the one live region and is written only when the words change, so a clock step does not announce September again. Below md it is full width under the row it belongs to (`calc(100vw - 16px)`, the material's `min-width` floor taken off). **One object, one animation**: the material's materialize keyframes are off for the sheet form in both directions, so the arrival rises on `SPRING_SHEET` alone and the dismissal continues off the bottom on the same spring rather than dissolving where it stands over the list. What holds the node while that exit plays is a keyframe that draws nothing, at the spring's own 300ms. The composer's grip carries the drag (`use-sheet-gesture.ts`, shared with the mail composer) and a release short of both thresholds returns it on `SPRING_SHEET_GESTURE`, the one preset with bounce, because that one had a finger behind it. The body rides `ScrollEdge variant="fade"` capped on `--radix-popover-content-available-height` less the material's 12 and `--tabbar-reserve`, so a short window scrolls the grid instead of putting Done past the bottom edge. **It returns DOM and React wraps it**, because the note's promote popover is a ProseMirror widget with no React tree to hang a component from: one drawing, two hosts, the `tasks-checkbox.tsx` arrangement. **`input[type=date]` and `input[type=time]` are banned under `app/`, `components/` and `lib/`**, on a pointer and on touch alike (D4 reverses the 0.10.0 ruling that touch keeps the native control), and `ops/design-guardrails.test.ts` holds it. |
| Tasks row selection (D5) | `.brain-tasks:is(:focus-within, :has([data-state="open"])) .brain-task-row:not([data-expanded]) > .tree-row-capsule`, `.brain-task-row > .tree-row-capsule`, `html[data-kbd] .brain-task-row[data-selected]`, `html[data-kbd] .brain-task-row:focus-visible` (`tasks-row.tsx`, `TASKS_SELECT_LAYOUT_ID`) | **The selection is a place, not an emphasis.** A whole column of tasks is one weight of one size, and the grey plate under the row the arrows stood on read as the row being important rather than as the row being current. The capsule keeps its shape and its flow between rows on `SPRING_SELECT` through a shared `layoutId` (dropped under reduced motion, so it appears in place) and draws at `--fill-glass-hover`, the tint the sidebar's own rows take on hover: enough to find, not enough to shout. The title keeps its weight, so nothing on the line moves when the capsule arrives. **The cursor is drawn while the column is holding the focus**, and the fill is on the column's state rather than on the capsule's own box: a press somewhere else on the page takes the paint and leaves the place, so a Tab back finds the capsule on the same row. A panel the column opened counts as the column, since Radix moves the focus into a portal at the end of the document and `data-state="open"` on the trigger is inside the column; an expanded row is excluded by name, because it wears the one fill of I1. **The row is what holds that focus**: `tabIndex={-1}`, script-focusable and out of the Tab order, the roving convention the picker's own grid cells use. Nothing in the column could hold it before, so a press on a row sent the focus up to `.brain-main` and `:focus-within` was false in the one case it has to be true. Keyboard focus adds the ring as everywhere else, `outline-offset: -3px` so it cannot clip on the column's edge, gated on `html[data-kbd]`; a focusable row answers the global ring at +2 as well, and two outlines on one capsule is two rings, so `html[data-kbd] .brain-task-row:focus-visible` overrides the OFFSET alone and the colour and the width stay the global's. **An expanded row folds on a press that lands outside it**, and a press is a pointer that goes down and comes back up within `PRESS_SLOP_PX` (8) of where it landed: `pointerdown` is the first event of a touch scroll, so a fold spent on the way down took the row with every finger dragged past it. The decision is still taken on the way down, where a layer the row opened is still flagged to be seen, because the press that dismisses a layer belongs to that layer and the row is the next press. A fold the reader pressed for leaves the focus where the press put it; every other fold hands it back to the row, so Escape from a chip cannot take the paint away with the chips and leave the reader neither fill nor ring. The selection is a roving attribute, the way the palette's is. A selected row's hover fill steps aside rather than stacking a second tint on the first, through `:not(:where([data-selected]))`, which weighs nothing, so the completed and history refusals go on beating it on order. **An expanded row is ONE layer.** It painted `--blue-tint-2` plus a `--blue-rim` hairline with the capsule still stacked under it, three fills on one row, and since a row cannot be expanded without being selected the quiet version above was never what a reader saw. It takes `--blue-tint`, the list-row value of §8, once, and the capsule under it draws nothing. **The tail reserves its 16px glyph slot on every row**, with or without a glyph to put in it, so a row carrying `restart` does not push its clock 22px left of where a clockless row ends: one right-aligned column down the axis a reader scans. **A completion does not travel.** The struck row fades out where it stands over `DUR.base`, the rows below close the gap on the layout spring, and it fades in at the foot of its group; it used to ride that spring down the group at full opacity and print itself over the rows it passed, so an open task briefly wore a filled box and a strike. Reduced motion reorders instantly and fades nothing. |
| Tasks head | `<TasksListTitle view categories titleRef onSelect>` and `<TasksListMenu>` (`tasks-list-menu.tsx`), `.brain-tasks-title-block`, `.brain-tasks-title`, `.brain-page-meta`, `.brain-tasks-head > .toolbar-pill` | **The column has the head a note has.** It started at the pill band plus 8, so the first thing on it was a row and a short list hung off the top of the screen with nothing to read it against; it starts at `--canvas-top` now, the line a page title sits on, 76 on desktop and 72 on a phone, on the same 700 measure. The first line is the list's own word in the Title register — Today, Upcoming, Inbox, Someday, Logbook, or the category, "No category" when the word is empty. **That word IS the list switcher**: the same menu the band's pill opens, the same rows, the same `onSelect`, drawn as a quiet button with the breadcrumb's marker weight (`alt-arrow-down-linear` 16 at ink-4) after it, hovering on the ink tint at r8 and pressing on the product's `scale(.97)`. It comes first in the column, so Tab reaches the switcher before the capture row. **Two `Dropdown.Root`s share one menu body**, because a Root has exactly one Trigger and whichever of the pair hosted it is the one Radix returns focus to on close — including while it is `visibility: hidden` and out of the tab order, which is a focus landing on nothing. Only one of the two is ever reachable: the title while it is on screen, the pill once it is not. **One caption, and only Today has one** — the weekday and the full month, "Tuesday, 16 September" (`longDayLabel`), on `.brain-page-meta`'s own spacing (min-height 20, margin −8 0 12), since Today is the one word here whose meaning is a date. Every other list and every category carries no caption and no reserved space, and no count: the group headers already say "Today · 2". **The band's pill waits**, on the lone breadcrumb's rule shared by selector rather than copied (§12 → Breadcrumb): hidden at rest, out of the tree and the tab order, still holding its box, materialising on opacity + scale .96 → 1 over 220ms `--ease-out` and retraced in 120ms, opacity alone under reduced motion, keyed on `data-title-out` written to `.brain-tasks` by `useTitleReveal` (`shell/use-title-reveal.ts`, the note's own observer, which now takes its scroller and its flag host as selectors). **The column had to be told it is the thing that scrolls** for any of that to hold: it asked for `min-height: 100%` inside a wrapper of indefinite height, so the percentage resolved to nothing, the surface grew to its content and the shell's scroller moved instead — which carried the floating head, its scroll edge and the pill away the moment a list ran past one window. It takes the window's own height, the way Mail's frame does, and **the head moved inside the scroller with it**: as a sibling of the scroller it was a hole in the wheel's scroll chain, because the pill is a hit target from the moment it is up — which is the moment the reader is scrolling — and from it the chain reached nothing that scrolls. It floats on the scroll edge's own construction now, `position: sticky` at the inset with a negative margin cancelling its own height, so it stands where it always stood and takes no box in the flow. The top edge stays where §7 put it (`--task-chrome` + inset + 28): the band is chrome and the rows pass under it, and the column reserves that band as its own `scroll-padding-top` (the band + 16, the figure `.brain-page-scroll` reserves from `--chrome-top`), so nothing the browser scrolls into view lands under the pill. |
| Notification bell and centre | `<NotificationsBell onNavigate refreshToken>` in the sidebar head (`notifications-bell.tsx`, `.brain-bell-badge`) | **The head holds a second control now, and only one: the bell, inboard of the accent circle**, so the circle keeps the corner it has always had. **`IconButton size={34}`, the circle's own size, with 12 of air between them** (`gap-3` on the pair). It stood at 28 with `gap-1`, so two controls that mean the same kind of thing were drawn a fifth apart in size and 4px apart on the line: what a reader saw was one lopsided object, and the smaller half of it was the half carrying a number. The row is still 36 and the panel still 280: 280 − 24 of panel padding − 6 of head padding leaves 250, the pair takes 34 + 12 + 34 = 80, the wordmark's box measures 74.4 at 1440, so 95.6 is spare and the wordmark is the only thing there that can grow (measured, not derived). `bell` at 17, `linear` at rest and `bold` while anything is unread. The count rides `.brain-bell-badge` at **`top: 0; right: 0`, the 34 box's own corner**: at 28 it hung 3px out on both sides to clear the crown, and the 34 box IS that hang, so the clearance measured on the 28 is unchanged while the badge stops reaching into the air beside New page. `--ink` on `--paper` at the pill radius over a 2px `--paper` ring (two ink shapes stay two; the head sits on translucent glass, so a ring drawn from the material would blend into whatever is behind it), `pointer-events: none` so the whole 34 box stays the press target, and it crossfades at `DUR.fast` keyed on the number, the construction the sidebar's Tasks count already uses one panel down and for the same reason: it changes while the reader is looking elsewhere, and a number that swaps in one frame reads as a glitch. It draws nothing when nothing is unread, so the head's resting shape is what it always was, and it caps at `99+`, past which the number is wider than the glyph it rides and says nothing "a lot" does not. The bell stands on Settings too, where the accent circle does not: "nothing there is a create" is an argument about creating, and a reminder fires whatever surface is open. There is no red anywhere in it, because §2 keeps a red fill out of the chrome, so the bold glyph is the second signal. **The centre is a menu, not a surface and not a dialog**: a short list of things that happened, each one a way into the place it happened. `brain-menu` at 320 with `brain-menu-item` rows, the kind's glyph in `brain-menu-icon` (`alarm-linear` fired, `clock-circle-linear` missed, `letter-linear` mail), the title in ink while the row is unread and ink-3 once it is read, the body in ink-3 after it **on the same line** (the item is 32 across the whole product and a second line here would be a second row height in a menu system that has exactly one, and Home's mail rows already read a sender and a subject this way), the relative time tabular at the tail, and the list on `ScrollEdge variant="fade"` capped on `--radix-dropdown-menu-content-available-height` so 500 rows scroll inside the material rather than under the fold. **New mail is ONE row in it, and it counts**: "10 new messages", no body, the letter glyph, opening Mail on its main screen and naming no thread. It was one row per thread until 0.12.2, so a morning's mail was thirty rows to be pressed one at a time and the centre had become a second inbox — the count grows while the row is unread and resets the moment Mail is opened, however it was opened, and stays reset for as long as Mail is on screen, because a poll that lands during an hour in Mail is about letters already in the list. The badge is still rows and not letters: how many things want you, with the row itself saying how many letters are behind that one. "Mark all read" is a row under a `brain-menu-sep`, drawn only while something is unread, because a control that would do nothing is worse than an absent one. **Below md there is no bell and no rows**: the tab slots are full and stay full, so the centre is a desktop object. Push is the phone's signal, Mail's own block on Home already names the new letters, and nothing stands above Home's capture field at any width. Home carried up to three unread rows there for one release, which put the centre's list where the page's opening line belongs and asked the reader to read a list before writing a line. |
| Dialog / sheet | `.brain-dialog` (+ `.brain-sheet` for the mobile form), `ConfirmDialog`, `DialogHeader`, `DialogBody` (`ui/confirm-dialog.tsx`, `ui/dialog-header.tsx`) | thick material r20, no border, L4 scrim (`--scrim-v2`), `dialog-pop` materialize with the sheen + edge-light at 60%, `dialog-pop-out` to leave and `dialog-commit` (`data-commit`) where the dismissal committed something (§6 → Motion); header Subheading 17 without a hairline — the body is `DialogBody`, the fade scroller (12px top once scrolled, 20px bottom while content continues); close = IconButton 28; buttons are `quiet` / `ink` / `destructive` (rename, move, trash, confirm); **list rows are `.brain-dialog-row`** — the palette row's construction on the same material: the r10 control the sheet's radius is derived from, at the body's 10px inset, 36 tall, Control 13/500 in ink, hover the ink tint, the chosen one the white capsule at 600, glyph and title the tree row's atoms so a page emoji stands off its title on the tree's 7px (Move's destinations, History's versions — both used to be r6 on an ink tint with the emoji touching the word); a search inside a dialog is the `Field` atom on glass; two-pane sheets keep the navigation on the material and put the main pane on paper (settings content, history preview — text lives on paper). Mobile sheets keep their slide; the full-screen settings page stays paper. **On `/share` the same dialog stands on paper**, not the thick material (`.brain-share-dialog`, the visitor's naming step in `share-subpage-dialog.tsx`): ban 10 keeps glass off the public page, and a share link is opened in the browsers whose `backdrop-filter` cannot be relied on. It is not the dialog class with a material painted over, so it carries no v2 ink flip either — the page under it is on the same ink. Radius, centring, the phone's `.brain-sheet` slide, the scrim, both retraces, the reduced-motion crossfade and print are shared by selector rather than copied, so the two cannot drift apart. |
| Share card | `<SharePopover isPublic pageId pageTitle hasPassword hasEdit expiresAt inheritedFrom expiredInheritedFrom scopeRevision onPrepareShare onEnableShare onAbsorbNestedShares onDisableShare onCopyLink onOpenShareSettings onSetProtection onSetEditable>` (`share-popover.tsx`, `.brain-share-plate` inside `.brain-share-popover`, and inside the `.brain-dialog`/`.brain-sheet` bottom sheet below sm) | **a ledger on glass**: one status sentence at the head (`.brain-share-head`, the Subheading register), then a row per fact or setting on a hairline (`.brain-share-row` — label Table 500 ink-2 at the left, the fact or the control in ink at the right, a note in Caption ink-3 under it), and the action last (`.brain-share-row-act`, right-aligned, the one ink fill on the surface). **The overlap state is a state of that same ledger, not a message in place of it**: the rows are replaced by what already overlaps this scope — one line per root, its title, whether it is a shared parent or a shared nested page, and `· expired` where it is — and the action row says what can be done about it. Where every overlap is **nested inside** this page, that is one primary action, `Share <title> instead`, over a sentence that states the whole consequence in the reader's terms, the widening as well as the reassurance ("Furniture's link will open inside this one and show everything Apartment shares; anyone who has it keeps it."), and `Review shared links` stays beside it as the quiet second; the press folds the nested grants into this page and the ledger turns into the ordinary shared state, since there is nothing left to resolve. Where the overlap is a **parent's** grant, or a live nested link carrying a gate this page does not — a password or a deadline — the ledger keeps `Resolve the existing grant before creating this link.` and the lone quiet button: the first is already the authority for this page, and the second would have its gate taken away by the fold, so the owner lifts it where it stands. A title too long for the capsule is cut at 24 code points, which keeps a surrogate pair whole. |
| Command palette | `.brain-palette` on the desktop `Dialog.Content`, `.brain-palette-sheet` on the phone (`command-palette.tsx`) | thick r22, 560 × ≤ 60vh at 12vh, input Subheading 17, group headings Label 11, cmdk's list is the fade scroller (`useScrollEdge` with both sentinels: the first row dissolves under the input once scrolled, the bottom edge goes at the end of the list), selected row the white .78 capsule r10 — **the plate is a cursor, so it needs one**: cmdk parks its selection on the first row as the list renders and moves it on `pointermove`, which makes the plate the row hover on a mouse and the arrow cursor on a keyboard, and nothing at all on a phone, where it used to park on "New page" for the whole session. It paints under `(hover: hover)` or `html[data-kbd]`, and a finger gets `:active` instead. Materialize on `data-state`. **On a phone it is a section, not a panel**: the Pages surface's construction — the canvas ground over the whole screen, arriving on the canvas's own `pageTransition` (a crossfade under reduced motion), the Radix content the transparent focus scope — with a 44px head (H3 "Search" centred, no Back row: the surface owns a history entry, so Back, the edge swipe and the Home tab all leave it), the `Field` atom on glass as its capsule (16px input, so iOS never zooms), the same headings and rows by the same two rules, the same fade scroller, and the floating tab bar and the New circle over its foot with the positions and material they wear on every other tab, cleared by a reserve at the end of the list. It stays glass-free because the bar above it is glass and a second backdrop-filter under that one is §10 ban 1, and because a list of text on a blurred layer is ban 2. It was a thick sheet on the 8px inset at r20 rising on `SPRING_SHEET`, with the app showing through the gutter on all four sides and the bar riding inside it as a plain row; before that it was a paper page of its own with uppercase 11/500 labels, r6 rows on an ink tint and a "← Editor" header no other surface on the phone said. |
...
| Agent rows (Settings → Connections) | `ConnectionsSection` (`settings/connections-section.tsx`), `MailRow`'s `sentByAgent` (`mail-row.tsx`) | Three additions in the surface's existing grammar, no new atom. **A grant names what it can reach**, not the words stored on it (`ownerEffectiveScopes`), so a connection minted before 0.11.0 reads "Read · Write · Import · Mail · Send mail" on the one screen a person revokes from. **What agents may do** is two rows of the v2 segmented control, the construction Notifications already uses for its kinds: "Tell recipients when an agent writes" resting Off and "Let agents send mail" resting On, each writing once, disabled while that write is out, and each toast naming the new rule rather than reporting that something saved. **Agent activity** is the last fifty lines an agent wrote, newest first, in the lead-row shape the grants above already use: client · tool in Table, then the change, the account's head and the outcome in Caption under it, `formatAgo` compact and tabular at the tail. Clear sits in the group header and is disabled while there is nothing to clear, and the `Empty` atom holds the ring when the log is empty or unreadable, the same rule that keeps a settings group from collapsing into a stray hairline. No row can carry a subject, an address or a title, because `lib/mcp/activity-log.ts` writes no field one could enter through. **In Mail**, a Sent row an agent put on the wire carries "Sent by ‹app›" in Caption at ink-3 on the second line, left of the paperclip and in the account word's register. Only in the Sent mailbox, resolved once as that mailbox opens and never polled: a thread the agent sent that has since been replied to is a conversation, not an agent's message. A row from a first-party SMTP account never carries the caption, because the mark has no provider thread to join on. The tools those lines come from, what each one may reach and the exact words of every refusal are `docs/mcp-tools.md`. |
...
menu's word, not "here". The two adds that carry no word are the same accent
circle making the same page: the sidebar's at 34, whose label, tooltip and ⌘⌥N
all say New page, and the phone's at 54, standing at the right inset beside the
tab bar and answering to the same name. What those two carry is the mark for
the ACT, and `document-text`, the noun, is a slot away in the bar, under the
word "Pages". **`/dev/glass` draws what
...
`confirm-discard-light.png` has two on screen — the sidebar's New circle
