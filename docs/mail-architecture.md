...
| Capacity | Separate receive, 17 MiB send, relay-frame, parser, connection, queue, cache, temp, WAL, and process limits sized for a handful of accounts on a small host shared with the rest of Brain |
...
`PATCH /v1/sync { "enabled": boolean }` is the owner's Mail switch, and it is
on the socket because the setting is in a file this process cannot read: the
unit runs as `brain-mail` and `/var/lib/brain/settings` is not readable to it.
The body is exactly that one boolean; anything else is a 400. Off stops all
three workers — the background sync scheduler, the outbox drainer and the SMTP
submission worker — and each of them aborts its provider call and then awaits
the pass in flight, so a pass mid-fetch drains rather than tears. While the
service is paused, `POST /v1/sync`, `POST /v1/send` and
`POST /v1/drafts/:draftId/send` are refused `409 sync_paused` before admission,
so no reservation is taken and no outbox row is written; every read, including
`GET /v1/send/:operationId`, answers as it always did. Nothing is deleted: not
an account, not a credential, not a cached body, and not a queued outbox row,
which stays queued and delivers when the switch comes back on. Commands are
serialized, so a resume that arrives while a pause is still draining runs after
it rather than being dropped. The answer is `{ "apiVersion": 1, "paused":
boolean }`.

The flag is durable, as a `sync_paused` row in the service's own `meta` table,
and the service honours it at startup instead of syncing and sending for one
interval after every restart. The startup record on stdout carries what that
start actually did, as `"phase": "running"` or `"phase": "paused"`. A missing
row, and any value the service did not write, read as running: the fail-open is
...
request or response may use up to the 24 MiB `maxDraftBodyBytes` admits, because
a valid 1 MiB text body can expand substantially when JSON escapes control bytes
and an attachment send carries 13.68 MiB of base64 beside it.
...
The read-back column stops where the provider starts. The drain column is the worker's own pass over a backlog, through the body the Gmail adapter builds, and it is flat in the depth of the queue:

| Backlog at 10 MiB | One pass |
| ---: | ---: |
| 20 (the default batch) | **197.2 MiB** |
| 100 (`maxQueuedSubmissions`) | 195.7 MiB |

That it is flat is the point: the listing carries identities and the worker reads one message at a time. Before that, twenty rows at the 8 MiB cap of the day held 219 MiB of message and measured 274 MiB resident, past the `MemoryMax` of the day. The level came down with the Gmail body too: built into one buffer a chunk at a time rather than `JSON.stringify`'d, which took the same twenty-deep pass from 206 MiB to 154 MiB at that cap.

The worst column of each row is the one the contract has to hold: `MemoryHigh=232M` and `MemoryMax=296M` in `ops/brain-mail.service`, and the bar is `MemoryHigh` less 15 MiB — 217 MiB — which is the room the service's own resident set needs above a bare node. It applies to every run of every stage, not to the worst stage alone. 10 MiB holds it with 19.8 MiB to spare, at the drain rather than at the enqueue, because the transport's cost per delivery overtook the enqueue's once the message stopped being copied into the row. 9 MiB holds it by 39.1 MiB and 11 MiB by 6.7 MiB, so 11 is headroom above this cap rather than the next cap. The deliver read-back is a later turn in the same process and never overlaps the build, so its own peak is not additive.

Every other outgoing cap follows from that number. Base64 at 76 columns multiplies a payload by 1.3684, so 10 MiB of files becomes 13.68 MiB of MIME parts. The text part costs more than its own limit, which is the term the first version of this arithmetic missed: `normalizeLineEndings` turns every line break into CRLF before the part is encoded, so the worst 1 MiB body — one made of nothing but line breaks — is 2 MiB before base64 and 2.74 MiB after it. The headers cannot reach their own 256 KiB from this composer: a hundred recipients at 254 bytes, a 998-byte subject as encoded words, ten Content-Disposition lines in both their ASCII and RFC 5987 forms, and the boundaries come to about 33 KiB. Built whole, that worst shape measures **16.45 MiB** (17,252,261 bytes) — under the 17 MiB `outgoingRawMessageBytes` states, with 0.55 MiB above it on purpose, because a ceiling a message at the cap can reach is a refusal after the build. `outbound-worker-integration.test.ts` builds exactly that message, through the store and out to the bytes the provider is handed, and asserts both that it is over 15 MiB and that it is inside the ceiling, so the ceiling is a figure a test can fail rather than one nothing reaches. The outbox checks a message against that figure by name, with both sizes in the refusal, before it writes a row. `MAX_SERIALIZED_SUBMISSION_BYTES` in `outbound-store.ts` is 1 MiB and no longer follows it at all: the row's JSON holds the ids, the envelope and the two digests, and 200 addresses of 254 bytes is about 51 KiB of that, about 102 KiB escaped at its worst. The JSON body that carries the request is capped at 24 MiB, which is the 13.68 MiB of base64 plus a 1 MiB text body escaped at its worst six bytes per source byte. That stays under the 30m `client_max_body_size` measured on the droplet, so the edge does not move with the cap. The note store's own 25 MiB per-file cap is a different limit on a different path and does not move with these.
