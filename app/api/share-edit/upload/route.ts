...
export async function POST(req: NextRequest) {
  ...
  return withShareWrite(
    ...
    async (ctx, store) => {
      // Decided from Content-Length before req.formData() buffers anything.
      // The owner route can check file.size after the parse because it sits
      // behind a session; on anonymous traffic that would mean up to sixty
      // bodies of nginx's 101 MB allocated per window on a 2 GB box. A
      // multipart body is a little larger than the file it carries, so a
      // file within a few hundred bytes of the cap is refused here rather
      // than after the parse: the safe side of the line.
