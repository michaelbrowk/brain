...
- **What floats is sized by its content, not by the window.** The inset says how close a floating object may come to the edge; it never says the object reaches it. Every pill in the product is shrink-to-fit — breadcrumb, toolbar, toast — and so is the **mobile tab bar**: six slots of `--tabbar-slot` plus the row's 4px ends, `width: fit-content` between the two insets with `margin-inline: auto`. **One measurement gives the slot**: the widest label is still "Search" at 37.6 (Label 11/500), and "Tasks", the word the sixth slot brought, measures 30.6 and does not take it. 9.2 of air on each side is 56.0, which is the 4px step. The sixth slot is what moved the air from 11.2 to 9.2 and the slot from 60 to 56, and the pair rule in the next bullet is what says 9.2 is still enough. Six of them plus the ends is 344, and the bar holds 344 at every window that leaves it that much between the insets (≥ 360): clearance 23 on each side at 390, 43 at 430, 48 at 440, which is 15, 35 and 40 past the 8px inset. It used to divide the full width into five equal parts, 73px slots holding a 38px word, and a run of five of those reads as a backing laid under the screen rather than as an object standing on it. Below 360 the window sets the width instead — `(w − 16 − 8) / 6`, which is 49.33 at 320 — and `minmax(44px, var(--tabbar-slot))` does not reach its floor until 288, where the bar is 272 and still sitting on both insets. Narrower than that it is wider than the space between them, `margin-inline: auto` centres it back through them, and at 272 it would reach the window. Nothing ships there. Measured per width by `e2e/mail-shots.spec.ts`.
- **Air around a word is not air between two words.** Two labels centred in equal tracks leave `track − (w₁ + w₂) / 2` between them, so the quantity to read is the widest adjacent **pair**, never the widest single label counted twice. The tab bar has three pairs now that Tasks stands between Search and New, and the New slot still carries a wordless 34px circle, so Search and Pages never stand side by side. At the 56 slot Home↔Search measures 21.1, Search↔Tasks 21.9 and Pages↔Mail 28.4, so the minimum is Home↔Search and the new neighbour does not overtake it. A 20px minimum would be met by 54.9, so the check passes from 354 up and not below it: at 354 the track is 55 and Home↔Search measures 20.1, at 353 it is 54.83 and 19.9, and at 320 the track is 49.33 and the gap is 14.4, against 24.3 on the five-slot bar. 320 is the narrowest phone the e2e set still shoots, so unlike the 288 floor this one lands on a width that ships, and it is the sixth slot's other measured cost. The 20 is a check the 56 slot passes and the window-sized bar below 354 does not. It is not where the slot comes from, and the first draft of the bullet above said it was, on arithmetic for a pair this bar does not have.
- **The canvas runs beside floating chrome, not only under it.** §7 gives the page canvas no edge band, so a paragraph passing the tab bar is drawn to the window's own edge and shows on both sides of the capsule as well as behind it — 3px of a word either side at 390, 23 at 430, 28 at 440, and 168 at 767, the last width before the bar is `display: none` and the sidebar takes over. The sixth slot took 18 off each of those strips, and at 375 it takes the strip away: the capsule closes over the 20px paragraph rule and covers the text by 4.5, clearing the rule again at 384. So the claim holds from 430 up, where the strip is wide enough to read as text standing beside chrome, and the 3px at 390 is the measured cost of the sixth slot. That cost is accepted. The full-width bar hid that, by accident of its width rather than by anyone's decision, and hid it least where it mattered most: a five-slot bar stretched across 767 is 751 wide with 150px slots, which is the failure this change is about, at its worst. Keeping it hidden costs either a band on the canvas (§7's named ban) or a bar sized by the window (the bullet above), and text passing beside chrome is already what this product does at the top of a full-width page, where the article runs under the toolbar pills. So it stays visible: it is the tab bar reading as an object standing on the screen that makes it so, and that is the change, not a side effect of it.
