...
folder of Markdown files you keep.
...
# fill in NOTES_ROOT, AUTH_SECRET, AUTH_PASSWORD_HASH and
# BRAIN_PUBLIC_ORIGIN in .env, then:
docker compose up -d
```
...
Open `http://localhost:3020` and log in with the password you hashed. The
session cookie is `Secure`, which Chrome, Firefox and Edge honour over
`http://localhost`; Safari does not, so on Safari put Brain behind HTTPS
before logging in.

`NOTES_ROOT` is the folder on the host that holds the notes. An empty one is
fine — Brain makes it a git repository on the first save — and it has to be
writable by uid 1000, the unprivileged user inside the image. `AUTH_SECRET`
wants 32 or more random bytes (`openssl rand -hex 32`). `AUTH_PASSWORD_HASH`
takes a bcrypt hash of your login password, which the Docker you already have
can produce:
...
set `BRAIN_PUBLIC_ORIGIN` to that hostname. `ops/nginx/brain.conf.example` is
that terminator, written for a host with nothing in front of it.
...
| Var                  | Purpose                                                         |
| -------------------- | --------------------------------------------------------------- |
| `NOTES_ROOT`         | Absolute path to the notes folder (git-backed source of truth)  |
...
| `BRAIN_PUBLIC_ORIGIN` | Exact public HTTPS origin used by Mail mutation checks and MCP OAuth discovery |
