...
- **No glass on glass.** A second `backdrop-filter` inside an element that has one is forbidden. The selected row in the sidebar (white .78 + `0 1px 2px`), pinned chips (white .50), kbd on glass — these are **fills**, not materials: `mat-fill-selected`, `mat-fill-chip`, `mat-fill-hover`. Text on a fill is 600 and full ink.
