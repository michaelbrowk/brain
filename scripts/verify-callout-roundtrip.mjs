...
async function importCallout() {
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "brain-callout-roundtrip-"));
...
const CASE2 = [
  ...
  "## Before you continue",
  "",
  "* Check the source markdown",
  "",
  "* Keep storage plain text",
  ":::",
].join("\n");
...
async function main() {
  ...
  const cases = [
    { name: "callout: paragraph", markdown: CASE1 },
    { name: "callout: heading + list", markdown: CASE2 },
  ];
