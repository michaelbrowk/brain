...
async function importToggle() {
  const sourcePath = join(repoRoot, "components/editor/toggle.ts");
...
const CASE1 = [
  ':::toggle{summary="Read more"}',
  "Hidden body text.",
  ":::",
].join("\n");

const CASE2 = [
  ':::toggle{summary="Launch checklist"}',
  "## Checks",
  "",
  "* Round-trip markdown",
  "",
  "* Run TypeScript",
  "",
  "[Related page](/p/abc123)",
  ":::",
].join("\n");

async function main() {
  const dom = await installDomGlobals();
  const loaded = await importToggle();
  const { toggle } = loaded.module;
  ...
  const cases = [
    { name: "toggle: paragraph", markdown: CASE1 },
    { name: "toggle: heading + list + link", markdown: CASE2 },
  ];
