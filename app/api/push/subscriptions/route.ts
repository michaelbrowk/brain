...
export async function POST(req: Request) {
  ...
  if (!body) return NextResponse.json({ error: "bad_subscription" }, { status: 400 });
  const record = parsePushSubscription({ ...body, at: new Date().toISOString() });
  if (!record) return NextResponse.json({ error: "bad_subscription" }, { status: 400 });
...
export async function DELETE(req: Request) {
  ...
  return NextResponse.json({ removed: await removePushSubscription(id) });
}
